...
const FIELDS_FOR_CHANGE = ['title', 'description', 'tags', 'lastModify']
...
export default class FirebaseDbWrapper {
    ...
    saveTheDay(id, day) {
        ...
        if (this.state && this.state.data && this.state.data.days && this.state.data.days[id]){
            if (FIELDS_FOR_CHANGE.filter((field)=>dayToSave[field] !== this.state.data.days[id][field]).length){
                hasItChanged = true;
            }
        ...
        return this.db.ref(`/userProfile/${this.user.uid}/days/${id}`).update(day).catch(() => {
