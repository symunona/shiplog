    ...
    constructor() {
        ...
        this.vue = new Vue({
            ...
            methods: {
                ...
                prevDay(days) { return this.dayList[this.prevDayId(this.currentDay.id, days || 1)] },
                thisDayLastYears(id) {
                    var days = {}
                    let yearAgo = 0
                    ...
                    while (yearAgo * 365 < todayIndex) {
                        yearAgo++
                        let dayId = this.prevDayId(this.currentDay.id, yearAgo * 365)
                        if (this.data.days[dayId]) {
                            days[dayId] = this.data.days[dayId]
                            days[dayId].yearAgo = yearAgo
                        }
                    }
                    return days
                },
                goNextDay() { this.changeTheDay(this.nextDayId(this.currentDay.id)) },
                goPrevDay() { this.changeTheDay(this.prevDayId(this.currentDay.id)) },
                changeTheDay(id){
                ...
                currentDay: {
                    handler(currentDay) {
                        if (currentDay) {
                            ...
                            this.lastYearList = this.thisDayLastYears(currentDay.id)
