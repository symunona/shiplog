...
class Shiplog {
    constructor() {
        ...
        let DayEditorComponent = {
            props: ['day', 'dayList', 'firstDayId', 'changeTheDay', 'saveTheDay'],
            created() {
                this.lastYearList = this.thisDayLastYears(this.day.id)
                setTimeout(this.resizeDescription, 100)
            },
            methods: {
                ...
                //     // this.saveDebounced(this.day, this.saveTheDay)
                // },
                // saveDebounced: _.debounce((day, saveTheDay)=>{
                //     console.log('debounce save the day', day.id, day)
                //     saveTheDay(day.id, day)
                // }, 2000),
            },
            watch: {
                day: {
                    deep: true,
                    handler (day) {
                        this.saveTheDay(day.id, day)
                        this.lastYearList = this.thisDayLastYears(day.id)
            ...
            watch: {
                filter: _.debounce(function(){
                        if (this.filter && this.filter.length > 1){
                            let found = {};
                            this.filter = this.filter.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()
                            Object.keys(this.dayList).map((id)=>{
                                if (SEARCH_FIELDS.find((field)=>
                                    matches(this.dayList[id][field], this.filter))
                                ){
                                    found[id] = this.dayList[id]
                                }
                            })
                            console.log(found)
                            this.filteredDayList = found
        ...
        this.vue = new Vue({
            ...
            methods: {
                ...
                changeTheDay(id) {
                    ...
                    if (id) {
                        if (id === this.currentDay.id) return;
                        location.hash = id
                ...
                onBackendDataChange(data){
                        ...
                        this.dayList = this.calculateDayList()
                        if (this.currentDay){
                            if (this.currentDay.description !== this.dayList[this.currentDay.id].description){
                                this.currentDay.description = this.currentDay.description + '\n[Conflict]\n' + this.dayList[this.currentDay.id].description
                            }
                        }
